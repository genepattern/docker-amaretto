...
amaretto_html_report <- function(AMARETTOinit,AMARETTOresults,CNV_matrix,MET_matrix,VarPercentage,hyper_geo_test_bool=TRUE,n_cluster=AMARETTOinit$NrCores,hyper_geo_reference_geneset_address='hyper_geo_test/H.C2CP.genesets.gmt',wordcloud_bool=FALSE)
{
    ...
    unlink("report_html/htmls/amaretto_data/*")
    unlink("report_html/htmls/modules_data/*")
    unlink("report_html/htmls/hyper_geo_data/*")
    unlink("report_html/htmls/hyper_geo_data/module_hyper_geo_test/*")
    
    dir.create("report_html")
    dir.create("report_html/htmls")
    dir.create("report_html/htmls/images")
    dir.create("report_html/htmls/amaretto_data")
    dir.create("report_html/htmls/modules_data")
    dir.create("report_html/htmls/hyper_geo_data")
    dir.create("report_html/htmls/hyper_geo_data/module_hyper_geo_test")
    ########################################################
    ...
    for (ModuleNr in 1:NrModules )
    {
      ...
      tmpfic<-HTMLInitFile("./report_html/htmls/",filename=fname,Title = tite_page,CSSFile="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css",useGrid = FALSE,useLaTeX=FALSE,HTMLframe=FALSE)
      ####### CSS ####
      ...
      all_gene_expression_file_address=paste("./report_html/htmls/modules_data",'/',all_gene_expression_file_name_save,sep="")
      
      write_gct(module_all_genes_data,all_gene_expression_file_address)
      ModuleData<-round(ModuleData,2)
     
      annotations_file_name_save=paste(module_name,"_","annotations",".tsv",sep="")
      annotations_file_address=paste('./report_html/htmls/modules_data','/',annotations_file_name_save,sep="")
      write_tsv(module_annotations,annotations_file_address,row_name = FALSE)
      #############
      HTML('<div class="row">',file=tmpfic)
            HTML('<div style="text-align: center">',file=tmpfic)
            #HTML('<br />',file=tmpfic)
            ...
            HTML(paste(
                       ...
                       ' Download Module ',as.character(ModuleNr),' Gene Annotations.tsv ',
                       '</a>',sep=""),file=tmpfic)
    ...
    ##############################################################################   
    tmpfic<-HTMLInitFile(address1,filename="index",Title = "Amaretto Report",CSSFile="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css",useGrid = FALSE,useLaTeX=FALSE,HTMLframe=FALSE)
    bootstrap1='<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">'
    bootstrap2='<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>'
    bootstrap3='<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>'
    ...
    write_gct(AMARETTOresults$ModuleData,'./report_html/htmls/amaretto_data/ModuleData_amaretto.gct')
    write_gct(AMARETTOresults$ModuleMembership,'./report_html/htmls/amaretto_data/ModuleMembership_amaretto.gct')
    write_gct(AMARETTOresults$RegulatoryProgramData,'./report_html/htmls/amaretto_data/RegulatoryProgramData_amaretto.gct')
    write_gct(AMARETTOresults$RegulatoryPrograms,'./report_html/htmls/amaretto_data/RegulatoryPrograms_amaretto.gct')
    write_tsv(AMARETTOresults$AllGenes,'./report_html/htmls/amaretto_data/AllGenes_amaretto.tsv')
    write_tsv(AMARETTOresults$AllRegulators,'./report_html/htmls/amaretto_data/AllRegulators_amaretto.tsv')
    write_tsv(AMARETTOresults$NrModules,'./report_html/htmls/amaretto_data/NrModules_amaretto.tsv')
    ...
    HTML(paste('<p class=".text-success text-left"> <font size="2"> Number of Genes (for ',as.character(VarPercentage),'% most variably expressed genes) = ',as.character(length(AMARETTOresults$AllGenes)),'</font></p>'))
    ...
    # HTMLEndFile()
    #################################################################################[####
    zip(zipfile = 'Amaretto_HTML_Report', files = './report_html')
    ###############################
}
...
create_gene_annotations<-function(module_all_genes_data,Module_Genes_names,Module_regulators_weights)
{
  ...
  df=data.frame(all_genes_names, targets_boolean, regulators_boolean,regulators_weight)  
  return(df)
}
...
write_gct<-function(data_in,file_address)
{
  ...
  column_one=rownames(data_in)
  column_two=rownames(data_in)
  data_mat<-cbind(column_one,column_two,data_in)
  
  
  full_data<-rbind(third_row,data_mat)
  write.table(data.matrix(full_data),file = file_address, sep="\t",quote = FALSE, col.names=FALSE, row.names=FALSE,append = TRUE)
  #return(data.matrix(full_data))
}
########################################################################################################################################
########################################################################################################################################
########################################################################################################################################
...
wordcloud_making<-function(text1,file_name){
  ...
  address=paste("report_html/htmls/images/",save_file_name,".svg",sep="")
